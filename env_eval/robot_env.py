...
import gym
from utils.random_env_test import random_env
from config import *
...
class ImitationLearning(gym.Env):
    def __init__(self):
        ...
        self.suction = None
        self.action_space = Box(low=-1, high=1, shape=(6,), dtype=np.float32)  # rad/s
        self.discrete_action = Discrete(2)  # {0, 1}
        ...
        self._read_specs_from_config(os.path.abspath(os.path.join(self.utils.find_project_root("HybrAJ"), '../asset/params/ur5_suction.xml')))
    ...
    def _load(self, config):
        ...
        self.robot_id = pybullet.loadURDF(os.path.abspath(os.path.join(self.utils.find_project_root("HybrAJ"), "../asset/ur5e/ur5e.urdf")), [0, 0, 0], flags=pybullet.URDF_USE_MATERIAL_COLORS_FROM_MTL)
        self.ghost_id = pybullet.loadURDF(os.path.abspath(os.path.join(self.utils.find_project_root("HybrAJ"), "../asset/ur5e/ur5e.urdf")), [0, 0, -10])  # For forward kinematics.
        ...
        for obj_name in obj_names:
            if obj_name not in list(fixed_destination.keys()):
                ...
                if object_type == 'block':
                    ...
                    object_position[2] = 0
                    object_id = pybullet.loadURDF(os.path.abspath(os.path.join(self.utils.find_project_root("HybrAJ"), f"../asset/{object_type}/{object_type}.urdf")), object_position,
    ...
    def reset(self):
        ...
        self._read_state()
        self.last_commanded_position = self.state[:6]
    ...
    def step(self, action):
        ...
        self._save_data(np.concatenate((continuous_action, discrete_action)), "action")  # save previous predicted actions and angles

        arm_joint_ctrl = np.clip(continuous_action, -1.0, 1.0)
        ...
        self._servoj(ctrl_feasible)
        self._do_simulation()
        self._read_state()  # Final state after control

        self.suction.activate() if bool(int(discrete_action[0])) else self.suction.release()  # suction de/activate
        self._do_simulation()

        # Get current data
        self._save_data([bool(discrete_action[0]), self._read_contact(self.info[0])], "contact_history")  # suction signal, contacting signal
        self._save_data(np.float32(pybullet.getLinkState(self.suction.body, 0)[0]), "ee_history")  # ee position
        ...
        if self.place_object_id is not None:
            self.history["target_obj"].append(
                self._read_info(self.obj_name_to_id[self.info[2]])
    ...
    def _velocity_to_position_control(self, velocity_action: np.ndarray) -> np.ndarray:
        ...
        clipped_velocity = np.clip(
            velocity_action, self.robot_vel_bound[:6, 0], self.robot_vel_bound[:6, 1]
        )
        ...
        safe_position_command = np.clip(
            target_position, self.robot_pos_bound[:6, 0], self.robot_pos_bound[:6, 1]
        )
