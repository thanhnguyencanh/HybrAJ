...
class Agent(object):
    ...
    def learn(self, memory, batch_size):
        ...
        # compute critic loss
        with T.no_grad():
            next_state_action, next_state_log_pi_c, _, next_p_action_logits, next_state_log_pi_d, _, next_p_action_id = \
            ...
            min_qf_next_target = T.min(qf1_next_target, qf2_next_target - self.alpha * next_state_log_pi_c - self.alpha_d * next_state_log_pi_d)
        ...
        pi, log_pi_c, _, p_action_logits, log_pi_d, _, p_action_id = self.actor.sample_normal(state_batch, reparametrization=True)
        ...
        alpha_loss = (-self.log_alpha * (log_pi_c.detach() + self.target_entropy)).sum(1).mean()
        alpha_d_loss = (-self.log_alpha_d * (log_pi_d.detach() + self.target_entropy_d)).sum(1).mean()
