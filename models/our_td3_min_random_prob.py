...
class Agent(object):
    def __init__(self, state_dim, action_dim, p_action_dim, max_action=1, lr=1e-4, epsilon=1.0, eps_min=0.05, eps_dec=1e-5,
        ...
        self.actor = Actor(state_dim, action_dim=action_dim, p_action_dim=p_action_dim, max_action=max_action, lr=lr).to(device)
        self.actor_target = Actor(state_dim, action_dim=action_dim, p_action_dim=p_action_dim, max_action=max_action, lr=lr).to(device)
    ...
    def learn(self, replay_buffer, batch_size=128):
        ...
        target_Q = reward + done * self.discount * torch.stack(target_Qs, dim=1).sum(dim=1)
