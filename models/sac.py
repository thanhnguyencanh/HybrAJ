...
class ActorNetwork(nn.Module):
    ...
    def sample_normal(self, state, reparametrization=True):
        ...
        log_prob_d = dist_d.log_prob(p_action_id).unsqueeze(-1)
...
class Agent(object):
    def __init__(self, input_dim, action_space, p_action_dim, lr=0.0001, epsilon=1.0, eps_min=0.05, eps_dec=1e-5,
        ...
        self.target_entropy_d = -np.log(self.p_action_dim)
        self.log_alpha = T.zeros(1, requires_grad=True, device=device)
        self.log_alpha_d = T.zeros(1, requires_grad=True, device=device)
        self.alpha_optim = optim.Adam([self.log_alpha], lr=a_lr)
        self.alpha_optim_d = optim.Adam([self.log_alpha_d], lr=a_lr)
    ...
    def learn(self, memory, batch_size):
        ...
        mask_batch = T.tensor(mask_batch, dtype=T.float32, device=device).unsqueeze(1)
        ...
        # compute critic loss
        with T.no_grad():
            next_state_action, next_state_log_pi_c, _, next_p_action_logits, next_state_log_pi_d, next_prob_d, next_p_action_id = \
                self.actor.sample_normal(next_state_batch, reparametrization=False)
            # next_p_action_id = self.actor.sample_discrete(next_p_action_logits,
            #                                                      epsilon=0.0)
            next_p_action_onehot = F.one_hot(next_p_action_id.squeeze(-1), self.p_action_dim).float()
            qf1_next_target, qf2_next_target = self.critic_target.forward(next_state_batch, next_state_action, next_p_action_onehot)

            # min_qf_next_target = T.min(qf1_next_target, qf2_next_target) - self.alpha * next_state_log_pi
            min_qf_next_target = (T.min(qf1_next_target, qf2_next_target) - self.alpha * next_state_log_pi_c
                                                 - self.alpha_d * next_state_log_pi_d)
            target_Q = reward_batch + mask_batch * self.discount * (min_qf_next_target)
            #  sum(1) -> sum over the second dimension, view(-1) flatten
        ...
        # policy_loss_c = (self.alpha * log_pi_c - min_qf_pi).sum(1).mean()
        # policy_loss_d = (self.alpha_d * log_pi_d - min_qf_pi).sum(1).mean()
        policy_loss = (self.alpha * log_pi_c + self.alpha_d * log_pi_d - min_qf_pi).mean()
        self.actor.optimizer.zero_grad()
        policy_loss.backward()
        self.actor.optimizer.step()

        alpha_loss = (-self.log_alpha * (log_pi_c.detach() + self.target_entropy)).mean()
        alpha_d_loss = (-self.log_alpha_d * (log_pi_d.detach() + self.target_entropy_d)).mean()
