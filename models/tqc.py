...
class Critic(Module):
    ...
    def forward(self, state, action, p_action_id):
        ...
        quantiles = torch.stack(tuple(net(sa)[0] for net in self.nets), dim=1)
    ...
    def train(self, replay_buffer, batch_size=256):
        for it in range(self.num_critic_updates):
            ...
            with torch.no_grad():
                ...
                sorted_z_part = sorted_z[:, :self.quantiles_total - self.top_quantiles_to_drop]  # dim [batch, critic*M - drop_num]
        ...
        # actor_loss = (self.alpha * log_pi_joint - self.critic(state, new_action, disc_onehot).mean(2).mean(1, keepdim=True)).mean()
