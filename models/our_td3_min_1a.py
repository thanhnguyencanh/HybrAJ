...
class Agent(object):
    ...
    def learn(self, replay_buffer, batch_size=128):
        ...
        target_Q1, target_Q2 = self.critic_target(next_state, next_action, next_p_action_onehot)
        ...
        # critic_bias1 = (current_Q1.detach() - target_Q).mean().item()
        # critic_bias2 = (current_Q2.detach() - target_Q).mean().item()
        # Q_mean = torch.stack([current_Q1, current_Q2], dim=0).mean(dim=0)
        # aggregate_bias = (Q_mean - target_Q).mean()
        # q_diff = (current_Q1 - current_Q2).abs().mean()
        ...
        if self.update_step % self.policy_freq == 0:
            ...
            for _ in range(20):
                ...
                acts.append(u.detach().cpu().numpy())
            acts = np.stack(acts)
            var_u = acts.var(axis=0)
            ...
            for p_a in range(self.p_action_dim):
                ...
                q1 = self.critic.Q1(state, joint_action, p_action_onehot)  # continuous branch come through Q,
                weighted_q = q1 * p_action_probs[:, p_a]  # discrete branch come through the expectation weighting of Q.
            ...
            self.writer.add_scalar("Actor/actor_var", var_u.mean(), self.update_step)
